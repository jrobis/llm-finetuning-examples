...
def load_dataset(cfg: DictConfig, logger):
    ...
    if cfg.model.name=='decapoda-research/llama-7b-hf':
        # tokenized_dataset = load_from_disk("/mnt/share/the_pile/tokenized_00")['train'].with_format(None).to_iterable_dataset()
        tokenized_dataset = load_from_disk("/mnt/share/the_pile/tokenized_val")['train'].with_format(None).to_iterable_dataset()
        logger.info(f"Type of tokenized dataset: {type(tokenized_dataset)}")
    else:
        status = True
        i = 0
        while status:
...
def main():
    ...
    # Set up dataloaders
    logger.info('Before tokenized dataset creation')
    tokenized_datasets = load_dataset(cfg, logger)
    logger.info('After tokenized dataset creation')

    if cfg.training.iterable:
        split_idx = cfg.training.eval_batch_size*cfg.training.max_eval_steps*cfg.training.gradient_accumulation_steps
        logger.info(f'split_idx: {split_idx}')
        train_dataset = tokenized_datasets.skip(split_idx)
        eval_dataset = tokenized_datasets.take(split_idx)
    ...
    # train_dataloader = DataLoader(
    #     train_dataset.to_iterable_dataset(),
    #     collate_fn=default_data_collator,
    #     batch_size=cfg.training.train_batch_size,
    # )

    # eval_dataloader = DataLoader(
    #     eval_dataset.to_iterable_dataset(),
    #     collate_fn=default_data_collator,
    #     batch_size=cfg.training.eval_batch_size,
    # )
    ...
    if cfg.training.iterable:
        logger.info(f"  Num examples = Unknown (iterable dataset)")
    else:    
    ...
    for epoch in range(starting_epoch, cfg.training.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step % cfg.training.eval_every == 0:
                ...
                logger.info(f'Length of losses array: {len(losses)}')
                # Removed for iterable dataset but idk why it's needed
        ...
        # Previously: len(train_dataset)/cfg.training.train_batch_size)
        # Previously: len(train_dataloader)
